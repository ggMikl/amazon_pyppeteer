...
from ProxyHandle import FilterProxy
from ProxyHandle import Proxy
...
#proxy
async def proxy_get(url,timeout,num):
    try :
        done = await FilterProxy('proxy_handle', proxy_host, proxy_port).filter_effective(url, timeout, num)
        return done
    except Exception as e:
        print(repr(e))

async def proxy_delete(proxy):
    await Proxy(proxy_host, proxy_port).delete(proxy)
...
async def craw_consume(task_name,shop_queue:asyncio.Queue,pool,lock,type_mode):
    ...
    while True:
        ...
        #proxy
        p_get_start = time.time()
        print(f'Task {task_name}: {shop_code}'+'获取proxy中………………')
        proxy = await proxy_get(url, 5, 1)
        proxy = proxy[0]#不这么写会报错
        p_get_end = time.time()
        proxy_time = p_get_end-p_get_start
        print(f'Task {task_name}: {shop_code} 获得:{proxy}')
        browser = await launch(headless=True,
                               args=['--disable-infobars',
                                     # f'--window-size={width},{height}',
                                     "--start-maximized",
                                     f"--proxy-server=http://{proxy}"#设置代理
                                     ]
                               )
        ...
        await browser.close()
        await proxy_delete(proxy)#这个proxy用完删除掉

        end = time.time()
        task_time = end - start - proxy_time
        print(f'Task {task_name}: {shop_code} 获得:{proxy},耗时{proxy_time:.2f} seconds')
